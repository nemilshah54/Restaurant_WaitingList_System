...
int main(int argc, char **argv)
{
	...
	// Code set for Debug mode. Debug mode gives information of entire list when -d command and userInput of d is given. 
	// It also gives information for name and group size waiting ahead during l command in main function.
	...
	node*head = NULL;      // Initializing the list node to null. It points towards nothing.
	...
	while ((ch = getNextNWSChar()) != EOF)
	{
		...
		{
			doList(head, ch, debugMODE);
		}
		else if ('d' == ch)
		{
			doDisplay(head, ch, debugMODE);
		}
		else if ('\n' == ch)
		{
